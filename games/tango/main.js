...
function hint() {
  ...
  paint();
  toast(removed
    ? `어긋난 칸 ${removed}개를 걷어 내고 다음 칸을 채웠습니다.`
    : '지금 알아낼 수 있는 칸을 채웠습니다.');
  finishIfDone();
}
...
function finishIfDone() {
  ...
  let note = `${game.size}×${game.size}, 놓인 칸 ${game.puzzle.given.length}개와`
    + ` 표 ${game.puzzle.links.length}개.`;
  if (game.hinted) {
