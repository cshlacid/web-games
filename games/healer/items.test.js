...
// --- 화면에 뜨는 숫자 ----------------------------------------------------
//
// 체력·마나·골드·경험치가 네 자리를 넘는 것이 흔한데, 쉼표가 없으면 자릿수를
// 세어야 읽힌다. **옵션 글자도 같은 함수를 거친다** — 화면마다 다른 모양의
// 숫자가 생기지 않게 하려는 것이다.
{
  check('세 자리까지는 그대로', D.num(999), '999');
  check('네 자리부터 끊는다', D.num(1000), '1,000');
  check('여러 번 끊는다', D.num(1234567), '1,234,567');
  check('음수도 끊는다', D.num(-4200), '-4,200');
  check('소수는 정수 자리만 끊는다', D.num(1234.5), '1,234.5');
  check('숫자가 아니면 그대로', D.num('알 수 없음'), '알 수 없음');
  check('옵션 글자도 같은 규칙을 탄다',
