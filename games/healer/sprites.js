...
const SHEETS = {
  ...
  // 수호자. **성기사와 같은 이유로 시안을 한 번 갈아 끼웠다** — 처음 것은 공격
  // 줄의 좌우가 칸마다 섞여 있었고 그림 자체가 애매했다. 지금 것은 짐승 가죽을
  // 뒤집어쓴 모습이라 **머리 위쪽이 그 가죽이고 인물의 얼굴은 그 아래에 있다** —
  // 방향을 볼 때 가죽의 짐승 얼굴을 인물 얼굴로 착각하지 않는다.
  tank: {
    src: 'tank.png',
    cell: { w: 101, h: 102 },
    cols: 8, rows: 4,
    ...
    // 세 줄이 다 오른쪽을 보는 시안이라 대기와 공격은 그대로 두고 걷기만 왼쪽을
    // 만들어 붙였다(성기사 새 시안과 같다).
    crops: {
      full: { x: 0, y: 0, w: 1, h: 1 },
      list: { x: 0.13, y: 0.13, w: 0.74, h: 0.84 },
      head: { x: 0.42, y: 0.18, w: 0.32, h: 0.32 },
    },
  },
