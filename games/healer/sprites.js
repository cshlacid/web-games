...
const SHEETS = {
  ...
  goblin: {
    ...
    crops: {
      full: { x: 0, y: 0, w: 1, h: 1 },
      list: { x: 0.09, y: 0.18, w: 0.84, h: 0.80 },
      head: { x: 0.22, y: 0.19, w: 0.50, h: 0.40 },
    },
  },
  ...
  // 성기사. **칸이 아군 중에 가장 크다**(130×119, 다음이 음유시인의 102×95).
  // 공격의 마지막 두 장이 인물만 한 빛의 초승달과 위로 솟는 빛기둥이라, 칸을
  // 인물에 맞춰 자르면 그것이 잘린다. **인물 키는 다른 아군과 같은 21.7칸**이고
  // (`box` 34는 칸 높이를 화면 칸으로 옮긴 값이라 인물이 커지는 것이 아니다),
  // 남는 자리는 전부 투명하다 — 규칙은 이 상자를 보지 않고 화면의 노드 폭만
  // 이것으로 정한다.
  paladin: {
    src: 'paladin.png',
    cell: { w: 130, h: 119 },
    ...
    crops: {
      full: { x: 0, y: 0, w: 1, h: 1 },
      list: { x: 0.25, y: 0.28, w: 0.58, h: 0.70 },
      head: { x: 0.35, y: 0.25, w: 0.28, h: 0.30 },
    },
  },
