...
// 화면에서는 팔 하나가 사라진 정도로만 나타난다.
{
  ...
  // 시안을 받은 일곱만 그림 파일이고 나머지 다섯(전사·고블린·주술사·오크·
  // 우두머리)이 도형이다. 화풍이 섞이는 것은 시안이 오는 대로 하나씩 갈아
  // 끼우기 때문이다 — 남은 시안이 오면 여기부터 바뀐다.
  check('도형 그림 다섯', Object.keys(Sprites.SPRITES).length, 5);
  check('그림 파일 일곱', Object.keys(Sprites.SHEETS).length, 7);
