...
// 화면에서는 팔 하나가 사라진 정도로만 나타난다.
{
  ...
  // 시안을 받은 여섯(주인공·수호자·궁수·마법사·도적·음유시인)만 그림 파일이고
  // 나머지 여섯이 도형이다. 화풍이 섞이는 것은 시안이 오는 대로 하나씩 갈아
  // 끼우기 때문이다 — 남은 시안이 오면 여기부터 바뀐다.
  check('도형 그림 여섯', Object.keys(Sprites.SPRITES).length, 6);
  check('그림 파일 여섯', Object.keys(Sprites.SHEETS).length, 6);
