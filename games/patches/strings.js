...
(function () {
  SharedI18n.add({
    ko: {
      ...
      'patches.hintCleared': '어긋난 조각 {count}개를 걷어 냈습니다.',
      'patches.why.cell': '밝힌 칸을 덮을 수 있는 조각이 이것뿐입니다.',
      'patches.why.clue': '밝힌 숫자가 가질 수 있는 조각이 이것뿐입니다.',
      'patches.why.owned.cell': '밝힌 칸들은 그 안의 숫자 차지라 다른 조각이 지나갈 수 없습니다. 그러면 남은 칸을 덮을 조각이 이것뿐입니다.',
      'patches.why.owned.clue': '밝힌 칸들은 그 안의 숫자 차지라 다른 조각이 지나갈 수 없습니다. 그러면 이 숫자가 가질 수 있는 조각이 이것뿐입니다.',
    ...
    en: {
      ...
      'patches.hintCleared': 'Removed {count} wrong patches.',
      'patches.why.cell': 'Only this patch can cover the highlighted cell.',
      'patches.why.clue': 'This is the only patch the highlighted number can have.',
      'patches.why.owned.cell': 'The highlighted cells belong to the number inside them, so no other patch can pass through. That leaves only this patch for the remaining cell.',
      'patches.why.owned.clue': 'The highlighted cells belong to the number inside them, so no other patch can pass through. That leaves only this patch for this number.',
    ...
    ja: {
      ...
      'patches.hintCleared': 'ずれたピース{count}個を取り除きました。',
      'patches.why.cell': '光っているマスを覆えるピースはこれだけです。',
      'patches.why.clue': '光っている数字が持てるピースはこれだけです。',
      'patches.why.owned.cell': '光っているマスは中の数字のものなので、ほかのピースは通れません。すると残りのマスを覆えるピースはこれだけです。',
      'patches.why.owned.clue': '光っているマスは中の数字のものなので、ほかのピースは通れません。するとこの数字が持てるピースはこれだけです。',
    ...
    'zh-CN': {
      ...
      'patches.hintCleared': '移走了 {count} 个切错的块。',
      'patches.why.cell': '能覆盖高亮格子的块只有这一个。',
      'patches.why.clue': '高亮数字能拥有的块只有这一个。',
      'patches.why.owned.cell': '高亮的格子属于其中的数字，别的块不能穿过。于是能覆盖剩下格子的块只有这一个。',
      'patches.why.owned.clue': '高亮的格子属于其中的数字，别的块不能穿过。于是这个数字能拥有的块只有这一个。',
    ...
    'zh-TW': {
      ...
      'patches.hintCleared': '移走了 {count} 個切錯的塊。',
      'patches.why.cell': '能覆蓋高亮格子的塊只有這一個。',
      'patches.why.clue': '高亮數字能擁有的塊只有這一個。',
      'patches.why.owned.cell': '高亮的格子屬於其中的數字，別的塊不能穿過。於是能覆蓋剩下格子的塊只有這一個。',
      'patches.why.owned.clue': '高亮的格子屬於其中的數字，別的塊不能穿過。於是這個數字能擁有的塊只有這一個。',
    ...
    es: {
      ...
      'patches.hintCleared': 'Se quitaron {count} piezas mal puestas.',
      'patches.why.cell': 'Solo esta pieza puede cubrir la casilla resaltada.',
      'patches.why.clue': 'Esta es la única pieza que puede tener el número resaltado.',
      'patches.why.owned.cell': 'Las casillas resaltadas pertenecen al número que tienen dentro, así que ninguna otra pieza puede pasar. Solo queda esta pieza para la casilla restante.',
      'patches.why.owned.clue': 'Las casillas resaltadas pertenecen al número que tienen dentro, así que ninguna otra pieza puede pasar. Solo queda esta pieza para este número.',
