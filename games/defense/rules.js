...
// 사유는 열쇠로 돌려준다. 문장은 화면 쪽 사전에 있다.
function canPlace(run, key, x, y) {
  if (run.over) return 'over';
  if (!D.UNITS[key]) return 'noSuchUnit';
  if (!run.roster.includes(key)) return 'notInTeam';
  if (x < 0 || y < 0 || x >= run.map.w || y >= run.map.h) return 'offBoard';
  const c = at(run.map, x, y);
  if (run.map.walls[c]) return 'wall';
  if (run.cells[c]) return 'occupiedByUnit';
  if (occupied(run, x, y)) return 'foeThere';
  ...
  // 그대로 둔다.
  if (x === run.map.entry.x && y === run.map.entry.y) return 'entry';
  if (D.UNITS[key].hero) {
    if (run.heroesUsed[key]) return 'heroUsed';
  ...
  if (run.gold < costOf(run, key)) return 'noGold';
  return null;
}
