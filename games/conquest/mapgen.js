...
// 지도를 만든다. 병력 번호 하나로 같은 지도가 다시 나오게 해서, 이상한 판을 만나면
// 번호만 들고 와 node에서 그대로 재현할 수 있게 했다.
//
// **한 판은 반드시 사거리 안에서 다 이어져 있어야 한다.** 끊긴 덩어리가 있으면 그쪽
// 거점은 영영 만질 수 없어 이길 수도 질 수도 없다. 이어졌는지 보고 아니면 그
// 병력을 버리고 다음 번호로 넘어간다 — 배치를 고쳐 붙이는 것보다 다시 뽑는 쪽이
// 훨씬 짧다.
(function () {
...
const H = 460;
const MARGIN = 30;
// 거점끼리 이만큼은 떨어뜨린다. 붙어 있으면 화면에서 두 개가 한 덩어리로 보이고,
// 사거리가 무의미해진다.
const MIN_GAP = 62;
...
function scatter(next, count) {
  const spots = [];
  // 뽑기를 무한정 돌리지 않는다. 자리가 모자라면 그 병력은 실패로 두고 다음 번호로
  // 넘긴다.
...
// 시작 자리는 서로 가장 먼 두 거점이다. 가까이서 시작하면 첫 시설을 건설도 전에
// 부딪혀 판이 3초에 끝난다.
//
// **다만 이웃이 하나뿐인 구석은 피한다.** 가장 먼 두 곳은 거의 늘 구석인데, 거기서
// 시작하면 갈 곳이 한 군데뿐이라 첫 몇 분 동안 고를 것이 없다. 이웃이 둘 이상인
// 거점끼리 다시 재고, 그런 짝이 없을 때만 원래 값으로 돌아간다.
function farthestPair(nodes) {
  const degree = new Map(nodes.map((a) =>
    [a.id, nodes.filter((b) => L.inRange(a, b)).length]));
  ...
  return pick(nodes.filter((a) => degree.get(a.id) >= 2)) || pick(nodes);
}
...
function build(seed, count) {
  ...
  if (!connected(nodes)) return null;

  const [mine, theirs] = farthestPair(nodes);
  for (const [id, owner] of [[mine, 1], [theirs, 2]]) {
    const a = nodes[id];
    a.owner = owner;
    a.builds = ['factory'];
    a.units = L.UNITS_PER_BUILD;
    // 시작 거점은 스탯을 가운데로 맞춘다. 뽑기에 따라 한쪽이 두 배 좋은 병력을
    // 들고 시작하는 판이 나오면 실력과 상관없이 갈린다.
    a.stats = { energy: 60, strength: 60, speed: 60 };
    a.r = Math.max(a.r, 16);
  }

  return { seed, width: W, height: H, nodes, start: { player: mine, enemy: theirs } };
}

// 병력을 하나 받아 쓸 만한 지도가 나올 때까지 번호를 올린다. 실패한 번호를 건너뛰는
// 것이 아니라 다음 번호를 쓰므로, 같은 병력은 늘 같은 지도가 된다.
...
if (typeof window !== 'undefined') window.ConquestMap = MapGen;
