...
// 실행: node games/conquest/ai.test.js
...
// **한두 판은 끝나지 않는데, 그것이 맞다.** 양쪽이 거점 하나로만 닿아 있는 지도가
// 나오면 앞줄의 크기가 각자의 생산량에 묶여(한도 초과분은 줄어든다) 어느 쪽도 상대를
...
// --- 판단 ---
// 지도에서 상황을 만들면 병력 번호에 따라 배치가 달라져 무엇을 보고 있는지 흐려진다.
// 여기서는 거점을 손으로 놓는다. 80만큼 떨어뜨리면 이웃이고, 160이면 사거리 밖이다.
...
function stage(spec) {
  const world = L.createWorld({
    ...
    nodes: spec.map((s, id) => ({
      ...
      owner: s.owner || 0, builds: s.builds || [], units: 0,
    })),
  });
  spec.forEach((s, id) => {
    if (s.units) {
      world.nodes[id].units[s.owner] = { n: s.units, str: 60, eng: 60, spd: 60 };
    }
  });
  return world;
}

const full = ['factory', 'turret'];

{
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: ['factory'], units: 12 },
    { x: 80, y: 0 },
  ]);
  check('병력이 열이면 시설부터 세운다', A.decide(w, 2), [{ type: 'build', id: 0, build: 'factory' }]);
}

{
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: ['factory'], units: 12 },
    { x: 80, y: 0, owner: 1, units: 5 },
  ]);
  check('적이 붙은 자리에는 방어탑', A.decide(w, 2), [{ type: 'build', id: 0, build: 'turret' }]);
}

{
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: full, units: 30 },
    { x: 80, y: 0 },
    { x: 0, y: 80, owner: 1, builds: ['turret'], units: 25 },
  ]);
...
{
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: full, units: 16 },
    { x: 80, y: 0, owner: 1, builds: ['turret'], units: 40 },
  ]);
...

{
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: full, units: 30 },
    { x: 80, y: 0, owner: 2, builds: full, units: 8 },
    { x: 160, y: 0, owner: 1, builds: ['turret'], units: 40 },
  ]);
...
{
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: full, units: 260 },
    { x: 80, y: 0, owner: 1, builds: ['turret', 'factory'], units: 180 },
  ]);
...
{
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: ['factory'], units: 30 },
    { x: 80, y: 0, owner: 1, units: 3 },
  ]);
  check('문을 닫아 두면 시설만 세운다',
    A.decide(w, 2, { hold: true }), [{ type: 'build', id: 0, build: 'turret' }]);
  check('방어를 꺼 두면 생산탑만 세운다',
    A.decide(w, 2, { hold: true, turrets: false }), [{ type: 'build', id: 0, build: 'factory' }]);
}

{
  // 앞줄 셋이 한 목표를 칠 수 있는 자리. 손을 한 번으로 묶으면 가장 큰 부대만 간다.
  const w = stage([
    { x: 0, y: 0, owner: 2, builds: full, units: 12 },
    { x: 0, y: 80, owner: 2, builds: full, units: 30 },
    { x: 0, y: 160, owner: 2, builds: full, units: 20 },
    { x: 80, y: 80, owner: 1, units: 6 },
  ]);
  check('제한이 없으면 앞줄이 한꺼번에 쏟아붓는다', A.decide(w, 2).length, 3);
  check('손을 하나로 묶으면 가장 큰 부대만 간다',
...

{
  ...
  check('쉬움에서만 방어탑을 접는다',
    [rung[0].turrets, rung[1].turrets, rung[2].turrets], [false, true, true]);
}
...
// **이겼는가가 아니라 앞섰는가로 본다.** 손이 느린 쪽은 이겨 놓고도 남은 거점을
// 다 걷어내는 데 한참이 걸려, 10분 안에 끝나지 않은 판이 흔하다. 그런 판도 거점을
  ...
  for (let t = 0; t < limit && !world.over; t += DT) {
    ...
    if (t >= tFoe) {
      ...
      A.apply(world, 2, A.decide(world, 2,
        { edge: foe.edge, hold: t < foe.opening, moves: foe.moves, turrets: foe.turrets }));
    }
    L.step(world, DT);
  }
