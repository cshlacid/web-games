...
//
// **칸마다 바탕·전구·점을 판을 만들 때 한 번 그려 두고 보이기만 바꾼다.** 끄는 동안
// 손가락 밑의 요소를 갈아 끼우면 조작이 끊긴다(저장소 규칙).
(function () {

const R = window.LightUpRules;
const S = window.LightUpSolver;
const G = window.LightUpGenerator;
const Sound = window.LightUpSound;
...
const LEVEL_KEY = 'web-games.lightup.level';
const BEST_KEY = 'web-games.lightup.best';
...
function build() {
  const { rows, cols, cells } = game.puzzle;
  ...
  for (let i = 0; i < rows * cols; i++) {
    ...
    const open = cells[i] === R.OPEN;
    rooms.push(svg('rect', { class: open ? 'room' : 'wall', x, y, width: U, height: U }, root));
    texts.push(null);
    bulbs.push(null);
    dots.push(null);
    if (open) {
      // 전구: 둥근 유리와 아래 받침.
      const g = svg('g', { class: 'bulb' }, root);
      svg('circle', { class: 'bulb-glass', cx: x + U / 2, cy: y + U * 0.44, r: U * 0.26 }, g);
      svg('rect', { class: 'bulb-base', x: x + U * 0.38, y: y + U * 0.68, width: U * 0.24, height: U * 0.14, rx: U * 0.04 }, g);
      bulbs[i] = g;
      dots[i] = svg('circle', { class: 'nobulb', cx: x + U / 2, cy: y + U / 2, r: U * 0.09 }, root);
    } else if (cells[i] >= 0) {
      // 세로 가운데는 기준선에서 0.35em 내려 맞춘다(슬리더링크와 같은 까닭).
      const text = svg('text', { class: 'clue', x: x + U / 2, y: y + U / 2, dy: '0.35em', 'font-size': U * 0.58 }, root);
      text.textContent = String(cells[i]);
      texts[i] = text;
    }
  }
  ...
  view = { root, rooms, bulbs, dots, texts };
...
function paint() {
  ...
  const clash = new Set(state.clash);
  for (let i = 0; i < marks.length; i++) {
    if (!view.bulbs[i]) continue;
    view.rooms[i].classList.toggle('lit', !!state.lit[i]);
    view.bulbs[i].classList.toggle('on', marks[i] === R.BULB);
    view.bulbs[i].classList.toggle('bad', clash.has(i));
    view.dots[i].classList.toggle('on', marks[i] === R.CROSS);
  }
  ...
  view.texts.forEach((node, i) => {
    ...
    node.classList.toggle('over', over.has(i));
  });
...
// 바꾼 것을 묶음으로 쌓는다. 끌어서 찍은 점과 힌트는 여러 칸을 한 번에 바꾸므로
// 칸 하나씩 쌓으면 되돌리는 데 여러 번 눌러야 한다.
function set(i, value, group) {
  if (game.puzzle.cells[i] !== R.OPEN) return false;
...
const SOUND_OF = { [R.BULB]: 'bulb', [R.CROSS]: 'dot', [R.EMPTY]: 'erase' };
...
// 누르는 순간 그 칸을 바꾼다. 끌면 첫 칸이 받은 점(또는 지우개)을 지나간 칸에 똑같이
// 찍는다 — 전구는 끌어 놓지 않는다. 끌어 놓게 두면 손가락이 지나간 줄이 통째로 서로
// 비추는 전구가 된다(Queens의 왕관과 같은 까닭). 벽은 건너뛴다.
el.board.addEventListener('pointerdown', (event) => {
  if (!game || game.done || !view) return;
  event.preventDefault();
  const i = cellAt(event);
  if (i < 0 || game.puzzle.cells[i] !== R.OPEN) return;
  const now = game.marks[i];
  // 빈칸 → 전구 → 점 → 빈칸. 오른쪽 단추는 점을 바로 찍는다.
  const value = event.button === 2
    ? (now === R.CROSS ? R.EMPTY : R.CROSS)
    : (now === R.EMPTY ? R.BULB : now === R.BULB ? R.CROSS : R.EMPTY);
  drag = { id: event.pointerId, value, last: i, group: [] };
  set(i, value, drag.group);
  const state = paint();
  Sound.play(value === R.BULB && state.clash.includes(i) ? 'clash' : SOUND_OF[value]);
  el.board.setPointerCapture(event.pointerId);
});

el.board.addEventListener('pointermove', (event) => {
  if (!drag || drag.id !== event.pointerId || drag.value === R.BULB) return;
  const i = cellAt(event);
...
function finish() {
  ...
  const note = [t('lightup.note', { size: game.puzzle.cols })];
  if (game.hinted) {
  ...
  SharedDailyUI.report('lightup', { level, size: game.puzzle.cols, time: game.elapsed, hints: game.hinted });
}
...
function hint() {
  ...
  for (let i = 0; i < game.marks.length; i++) {
    ...
    const wrong = (mine === R.BULB && sol[i] !== S.BULB) || (mine === R.CROSS && sol[i] === S.BULB);
    if (wrong && set(i, R.EMPTY, group)) removed++;
  }

  // 사람의 표시를 풀이기의 칸 상태로 옮긴다. 점은 "전구 아님"이다.
  const state = S.start(S.context(game.puzzle));
  ...
  if (step) {
    const value = step.value === S.BULB ? R.BULB : R.CROSS;
    for (const i of step.cells) {
      set(i, value, group);
      const node = view.rooms[i];
      node.classList.add('hinted');
      setTimeout(() => node.classList.remove('hinted'), 1400);
    }
    lines.push(t(`lightup.why.${step.why.code}`, { clue: step.why.clue }));
  }
  if (!group.length) return;
  game.hinted++;
  Sound.play('hint');
...
// 폭이 바뀌면 칸 크기를 다시 잰다. 판을 다시 그리되 놓은 전구와 점은 그대로 둔다.
...
window.LightUpDebug = { game: () => game, hint, newGame };
