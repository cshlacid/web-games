...
// 실행: node games/lightup/generator.test.js
//
...
for (const level of Object.keys(B.LEVELS)) {
  ...
  set.list.forEach((code, id) => {
    ...
    check(`${name}: 글자`, code.length === spec.size * spec.size && /^[.#0-4]+$/.test(code));
...

{
  ...
  check('고른 판의 크기', first.rows === 7 && first.cols === 7 && first.cells.length === 49);
}
