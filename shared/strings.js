// 여러 페이지가 함께 쓰는 문구만 둔다. 게임 이름은 목록 카드와 그 게임의 헤더·문서
// 제목에 같이 나오므로 여기 한 곳에서 관리하고, 게임 하나에만 나오는 문구는 그 게임
// 폴더의 strings.js에 둔다.
(function () {
  SharedI18n.add({
    ko: {
      'game.2048': '2048',
      'game.kkodle': '꼬들',
      'game.sudoku': '스도쿠',
      'game.doppelblock': '더블블록',
      'game.chess-puzzle': '체스 퍼즐',
      'game.zip': '한붓그리기',
      'game.queens': '왕관 놓기',
      'game.patches': 'Patches',
      'game.tango': 'Tango',
      'game.idioms': '사자성어 찾기',
      'game.conquest': '점령전',
      'game.hashi': '다리 잇기',
      'game.dicewars': '주사위 영토전',
      'game.yacht': '야추',
      'game.nonogram': '노노그램',
      'game.rushhour': '차 빼기',
      'game.healer': '힐러',
      'game.defense': '길목 지키기',
      'game.metro': '지하철 노선',
      'game.roadworks': '도로공사',
      'game.sokoban': '상자 밀기',

      'ui.back': '← 목록',
      'ui.bgm': 'BGM',
      'ui.sfx': '효과음',
      'ui.help': '도움말',
      'ui.close': '닫기',
      'ui.score': '점수',
      'ui.best': '최고',
      'ui.rules': '규칙',
      'ui.hint': '힌트',
      'ui.undo': '되돌리기',
      'ui.erase': '지우기',
      'ui.pencil': '연필',
      'ui.newGame': '새 게임',
      'ui.newBoard': '새 판',
      'ui.again': '한 판 더',
      'ui.retry': '다시 하기',
      'ui.restart': '다시',
      'ui.next': '다음',
      'ui.difficulty': '난이도',
      'ui.making': '만드는 중…',
      'ui.easy': '쉬움',
      'ui.medium': '보통',
      'ui.hard': '어려움',
      'ui.small': '작게',
      'ui.large': '크게',
      'ui.boardSize': '판 크기',
      'record.best': '최고 {time}',
      'record.done': '완성! {time}',
      'record.hinted': '힌트를 {count}번 썼으니 기록은 남기지 않습니다.',
      'record.improved': '최고 기록을 {time}에서 줄였습니다.',
      'record.first': '첫 기록입니다.',
      'record.bestIs': '최고 기록은 {time}입니다.',
      'record.genFail': '판을 만들지 못했습니다. 새 판을 눌러 다시 시도해 주세요.',
      'record.hintedShort': '힌트를 써서 기록에는 넣지 않습니다',
      'record.bestMark': '최고 기록',
      'ui.toStart': '처음으로',
      'ui.nextBoard': '다음 판',
      'ui.players': '인원',
      'ui.playerCount': '{n}인',

      'daily.toast': '오늘의 도전 {done}/{total}',
      'daily.toastAll': '오늘의 도전 완료!',
      'daily.sudoku.any': '아무 난이도나 한 판 풀기',
      'daily.sudoku.medium': '보통 이상 난이도 풀기',
      'daily.sudoku.fast': '{min}분 안에 풀기',
      'daily.sudoku.hard': '어려움 난이도 풀기',
      'daily.queens.any': '한 판 풀기',
      'daily.queens.big': '{size}×{size} 판 풀기',
      'daily.queens.clean': '힌트 없이 풀기',
      'daily.queens.fast': '{size}×{size} 이상을 힌트 없이 {min}분 안에 풀기',
      'daily.tango.any': '한 판 풀기',
      'daily.tango.big': '{size}×{size} 판 풀기',
      'daily.tango.clean': '힌트 없이 풀기',
      'daily.tango.fast': '{size}×{size} 판을 힌트 없이 {min}분 안에 풀기',
      'daily.nonogram.any': '{size}×{size} 이상 한 판 풀기',
      'daily.nonogram.clean': '{size}×{size} 이상을 힌트 없이 풀기',
      'daily.nonogram.big': '{size}×{size} 판 풀기',
      'daily.nonogram.hard': '{size}×{size} 판을 힌트 없이 풀기'
    },
    en: {
      'game.2048': '2048',
      'game.kkodle': 'Kkodle',
      'game.sudoku': 'Sudoku',
      'game.doppelblock': 'Doppelblock',
      'game.chess-puzzle': 'Chess Puzzles',
      'game.zip': 'One Line',
      'game.queens': 'Queens',
      'game.patches': 'Patches',
      'game.tango': 'Tango',
      'game.idioms': 'Idiom Trails',
      'game.conquest': 'Conquest',
      'game.hashi': 'Bridges',
      'game.dicewars': 'Dice Wars',
      'game.yacht': 'Yacht',
      'game.nonogram': 'Nonogram',
      'game.rushhour': 'Rush Hour',
      'game.healer': 'Healer',
      'game.defense': 'Hold the Line',
      'game.metro': 'Metro Lines',
      'game.roadworks': 'Roadworks',
      'game.sokoban': 'Box Pusher',

      'ui.back': '← All games',
      'ui.bgm': 'Music',
      'ui.sfx': 'Sound',
      'ui.help': 'Help',
      'ui.close': 'Close',
      'ui.score': 'Score',
      'ui.best': 'Best',
      'ui.rules': 'Rules',
      'ui.hint': 'Hint',
      'ui.undo': 'Undo',
      'ui.erase': 'Erase',
      'ui.pencil': 'Notes',
      'ui.newGame': 'New game',
      'ui.newBoard': 'New board',
      'ui.again': 'Play again',
      'ui.retry': 'Try again',
      'ui.restart': 'Restart',
      'ui.next': 'Next',
      'ui.difficulty': 'Difficulty',
      'ui.making': 'Generating…',
      'ui.easy': 'Easy',
      'ui.medium': 'Medium',
      'ui.hard': 'Hard',
      'ui.small': 'Small',
      'ui.large': 'Large',
      'ui.boardSize': 'Board size',
      'record.best': 'Best {time}',
      'record.done': 'Done! {time}',
      'record.hinted': 'You used {count} hints, so this one is not recorded.',
      'record.improved': 'Your best time is down from {time}.',
      'record.first': 'Your first record.',
      'record.bestIs': 'Your best time is {time}.',
      'record.genFail': 'Could not build a board. Tap New board to try again.',
      'record.hintedShort': 'hints used, so this one is not recorded',
      'record.bestMark': 'best time',
      'ui.toStart': 'Restart',
      'ui.nextBoard': 'Next board',
      'ui.players': 'Players',
      'ui.playerCount': '{n} players',

      'daily.toast': 'Daily challenge {done}/{total}',
      'daily.toastAll': 'Daily challenge complete!',
      'daily.sudoku.any': 'Solve a puzzle on any level',
      'daily.sudoku.medium': 'Solve on Medium or harder',
      'daily.sudoku.fast': 'Solve in under {min} minutes',
      'daily.sudoku.hard': 'Solve on Hard',
      'daily.queens.any': 'Solve a puzzle',
      'daily.queens.big': 'Solve a {size}×{size} board',
      'daily.queens.clean': 'Solve without hints',
      'daily.queens.fast': 'Solve {size}×{size} or larger without hints in under {min} min',
      'daily.tango.any': 'Solve a puzzle',
      'daily.tango.big': 'Solve a {size}×{size} board',
      'daily.tango.clean': 'Solve without hints',
      'daily.tango.fast': 'Solve {size}×{size} without hints in under {min} min',
      'daily.nonogram.any': 'Solve a {size}×{size} or larger puzzle',
      'daily.nonogram.clean': 'Solve {size}×{size} or larger without hints',
      'daily.nonogram.big': 'Solve a {size}×{size} puzzle',
      'daily.nonogram.hard': 'Solve {size}×{size} without hints'
    },
    ja: {
      'game.2048': '2048',
      'game.kkodle': 'コドゥル',
      'game.sudoku': '数独',
      'game.doppelblock': 'ドッペルブロック',
      'game.chess-puzzle': 'チェスパズル',
      'game.zip': '一筆書き',
      'game.queens': 'クイーン置き',
      'game.patches': 'Patches',
      'game.tango': 'Tango',
      'game.idioms': '四字熟語さがし',
      'game.conquest': '陣取り',
      'game.hashi': '橋渡し',
      'game.dicewars': 'ダイスウォーズ',
      'game.yacht': 'ヨット',
      'game.nonogram': 'お絵かきロジック',
      'game.rushhour': 'ラッシュアワー',
      'game.healer': 'ヒーラー',
      'game.defense': '進路防衛',
      'game.metro': '地下鉄路線',
      'game.roadworks': '道路工事',
      'game.sokoban': '箱押し',

      'ui.back': '← 一覧',
      'ui.bgm': 'BGM',
      'ui.sfx': '効果音',
      'ui.help': 'ヘルプ',
      'ui.close': '閉じる',
      'ui.score': 'スコア',
      'ui.best': 'ベスト',
      'ui.rules': 'ルール',
      'ui.hint': 'ヒント',
      'ui.undo': '戻す',
      'ui.erase': '消す',
      'ui.pencil': 'メモ',
      'ui.newGame': '新しいゲーム',
      'ui.newBoard': '新しい盤',
      'ui.again': 'もう一局',
      'ui.retry': 'もう一度',
      'ui.restart': 'やり直す',
      'ui.next': '次へ',
      'ui.difficulty': '難易度',
      'ui.making': '生成中…',
      'ui.easy': 'やさしい',
      'ui.medium': 'ふつう',
      'ui.hard': 'むずかしい',
      'ui.small': '小さめ',
      'ui.large': '大きめ',
      'ui.boardSize': '盤の大きさ',
      'record.best': 'ベスト {time}',
      'record.done': '完成！{time}',
      'record.hinted': 'ヒントを{count}回使ったので記録には残しません。',
      'record.improved': 'ベスト記録を {time} から縮めました。',
      'record.first': '初めての記録です。',
      'record.bestIs': 'ベスト記録は {time} です。',
      'record.genFail': '盤を作れませんでした。「新しい盤」を押してもう一度お試しください。',
      'record.hintedShort': 'ヒントを使ったので記録には残しません',
      'record.bestMark': 'ベスト記録',
      'ui.toStart': '最初から',
      'ui.nextBoard': '次の盤',
      'ui.players': '人数',
      'ui.playerCount': '{n}人',

      'daily.toast': '今日のチャレンジ {done}/{total}',
      'daily.toastAll': '今日のチャレンジ達成!',
      'daily.sudoku.any': 'どの難易度でも1問解く',
      'daily.sudoku.medium': 'ふつう以上の難易度で解く',
      'daily.sudoku.fast': '{min}分以内に解く',
      'daily.sudoku.hard': 'むずかしいで解く',
      'daily.queens.any': '1問解く',
      'daily.queens.big': '{size}×{size}の盤を解く',
      'daily.queens.clean': 'ヒントなしで解く',
      'daily.queens.fast': '{size}×{size}以上をヒントなしで{min}分以内に解く',
      'daily.tango.any': '1問解く',
      'daily.tango.big': '{size}×{size}の盤を解く',
      'daily.tango.clean': 'ヒントなしで解く',
      'daily.tango.fast': '{size}×{size}をヒントなしで{min}分以内に解く',
      'daily.nonogram.any': '{size}×{size}以上を1問解く',
      'daily.nonogram.clean': '{size}×{size}以上をヒントなしで解く',
      'daily.nonogram.big': '{size}×{size}の問題を解く',
      'daily.nonogram.hard': '{size}×{size}をヒントなしで解く'
    },
    'zh-CN': {
      'game.2048': '2048',
      'game.kkodle': '韩语猜词',
      'game.sudoku': '数独',
      'game.doppelblock': '双块',
      'game.chess-puzzle': '国际象棋谜题',
      'game.zip': '一笔画',
      'game.queens': '放皇后',
      'game.patches': 'Patches',
      'game.tango': 'Tango',
      'game.idioms': '找四字成语',
      'game.conquest': '占领战',
      'game.hashi': '数桥',
      'game.dicewars': '骰子领土战',
      'game.yacht': '快艇骰子',
      'game.nonogram': '数织',
      'game.rushhour': '停车难题',
      'game.healer': '治疗师',
      'game.defense': '守路口',
      'game.metro': '地铁线路',
      'game.roadworks': '道路改造',
      'game.sokoban': '推箱子',

      'ui.back': '← 列表',
      'ui.bgm': '音乐',
      'ui.sfx': '音效',
      'ui.help': '帮助',
      'ui.close': '关闭',
      'ui.score': '分数',
      'ui.best': '最高',
      'ui.rules': '规则',
      'ui.hint': '提示',
      'ui.undo': '撤销',
      'ui.erase': '擦除',
      'ui.pencil': '笔记',
      'ui.newGame': '新游戏',
      'ui.newBoard': '新一局',
      'ui.again': '再来一局',
      'ui.retry': '再试一次',
      'ui.restart': '重来',
      'ui.next': '下一个',
      'ui.difficulty': '难度',
      'ui.making': '生成中…',
      'ui.easy': '简单',
      'ui.medium': '普通',
      'ui.hard': '困难',
      'ui.small': '小',
      'ui.large': '大',
      'ui.boardSize': '盘面大小',
      'record.best': '最高 {time}',
      'record.done': '完成！{time}',
      'record.hinted': '用了 {count} 次提示，这一局不计入记录。',
      'record.improved': '最好成绩从 {time} 提升了。',
      'record.first': '这是第一条记录。',
      'record.bestIs': '最好成绩是 {time}。',
      'record.genFail': '没能生成盘面。点“新一局”再试一次。',
      'record.hintedShort': '用了提示，不计入记录',
      'record.bestMark': '最好成绩',
      'ui.toStart': '从头开始',
      'ui.nextBoard': '下一局',
      'ui.players': '人数',
      'ui.playerCount': '{n} 人',

      'daily.toast': '今日挑战 {done}/{total}',
      'daily.toastAll': '今日挑战完成!',
      'daily.sudoku.any': '任意难度完成一局',
      'daily.sudoku.medium': '以中等或更高难度完成',
      'daily.sudoku.fast': '{min}分钟内完成',
      'daily.sudoku.hard': '以困难难度完成',
      'daily.queens.any': '完成一局',
      'daily.queens.big': '完成{size}×{size}盘面',
      'daily.queens.clean': '不用提示完成',
      'daily.queens.fast': '{size}×{size}以上不用提示{min}分钟内完成',
      'daily.tango.any': '完成一局',
      'daily.tango.big': '完成{size}×{size}盘面',
      'daily.tango.clean': '不用提示完成',
      'daily.tango.fast': '{size}×{size}不用提示{min}分钟内完成',
      'daily.nonogram.any': '完成一局{size}×{size}以上',
      'daily.nonogram.clean': '{size}×{size}以上不用提示完成',
      'daily.nonogram.big': '完成{size}×{size}题目',
      'daily.nonogram.hard': '{size}×{size}不用提示完成'
    },
    'zh-TW': {
      'game.2048': '2048',
      'game.kkodle': '韓語猜詞',
      'game.sudoku': '數獨',
      'game.doppelblock': '雙塊',
      'game.chess-puzzle': '西洋棋謎題',
      'game.zip': '一筆畫',
      'game.queens': '放皇后',
      'game.patches': 'Patches',
      'game.tango': 'Tango',
      'game.idioms': '找四字成語',
      'game.conquest': '佔領戰',
      'game.hashi': '數橋',
      'game.dicewars': '骰子領土戰',
      'game.yacht': '快艇骰子',
      'game.nonogram': '數織',
      'game.rushhour': '停車難題',
      'game.healer': '治療師',
      'game.defense': '守路口',
      'game.metro': '地鐵路線',
      'game.roadworks': '道路改造',
      'game.sokoban': '推箱子',

      'ui.back': '← 列表',
      'ui.bgm': '音樂',
      'ui.sfx': '音效',
      'ui.help': '說明',
      'ui.close': '關閉',
      'ui.score': '分數',
      'ui.best': '最高',
      'ui.rules': '規則',
      'ui.hint': '提示',
      'ui.undo': '復原',
      'ui.erase': '擦除',
      'ui.pencil': '筆記',
      'ui.newGame': '新遊戲',
      'ui.newBoard': '新一局',
      'ui.again': '再來一局',
      'ui.retry': '再試一次',
      'ui.restart': '重來',
      'ui.next': '下一個',
      'ui.difficulty': '難度',
      'ui.making': '生成中…',
      'ui.easy': '簡單',
      'ui.medium': '普通',
      'ui.hard': '困難',
      'ui.small': '小',
      'ui.large': '大',
      'ui.boardSize': '盤面大小',
      'record.best': '最高 {time}',
      'record.done': '完成！{time}',
      'record.hinted': '用了 {count} 次提示，這一局不計入紀錄。',
      'record.improved': '最佳成績從 {time} 提升了。',
      'record.first': '這是第一筆紀錄。',
      'record.bestIs': '最佳成績是 {time}。',
      'record.genFail': '沒能產生盤面。點「新一局」再試一次。',
      'record.hintedShort': '用了提示，不計入紀錄',
      'record.bestMark': '最佳成績',
      'ui.toStart': '從頭開始',
      'ui.nextBoard': '下一局',
      'ui.players': '人數',
      'ui.playerCount': '{n} 人',

      'daily.toast': '今日挑戰 {done}/{total}',
      'daily.toastAll': '今日挑戰完成!',
      'daily.sudoku.any': '任意難度完成一局',
      'daily.sudoku.medium': '以中等或更高難度完成',
      'daily.sudoku.fast': '{min}分鐘內完成',
      'daily.sudoku.hard': '以困難難度完成',
      'daily.queens.any': '完成一局',
      'daily.queens.big': '完成{size}×{size}盤面',
      'daily.queens.clean': '不用提示完成',
      'daily.queens.fast': '{size}×{size}以上不用提示{min}分鐘內完成',
      'daily.tango.any': '完成一局',
      'daily.tango.big': '完成{size}×{size}盤面',
      'daily.tango.clean': '不用提示完成',
      'daily.tango.fast': '{size}×{size}不用提示{min}分鐘內完成',
      'daily.nonogram.any': '完成一局{size}×{size}以上',
      'daily.nonogram.clean': '{size}×{size}以上不用提示完成',
      'daily.nonogram.big': '完成{size}×{size}題目',
      'daily.nonogram.hard': '{size}×{size}不用提示完成'
    },
    es: {
      'game.2048': '2048',
      'game.kkodle': 'Kkodle',
      'game.sudoku': 'Sudoku',
      'game.doppelblock': 'Doppelblock',
      'game.chess-puzzle': 'Ajedrez',
      'game.zip': 'Un solo trazo',
      'game.queens': 'Reinas',
      'game.patches': 'Patches',
      'game.tango': 'Tango',
      'game.idioms': 'Rastros de modismos',
      'game.conquest': 'Conquista',
      'game.hashi': 'Puentes',
      'game.dicewars': 'Guerra de dados',
      'game.yacht': 'Yacht',
      'game.nonogram': 'Nonograma',
      'game.rushhour': 'Atasco',
      'game.healer': 'Sanador',
      'game.defense': 'Defensa',
      'game.metro': 'Líneas de Metro',
      'game.roadworks': 'Obras Viales',
      'game.sokoban': 'Empuja cajas',

      'ui.back': '← Lista',
      'ui.bgm': 'Música',
      'ui.sfx': 'Efectos',
      'ui.help': 'Ayuda',
      'ui.close': 'Cerrar',
      'ui.score': 'Puntos',
      'ui.best': 'Mejor',
      'ui.rules': 'Reglas',
      'ui.hint': 'Pista',
      'ui.undo': 'Deshacer',
      'ui.erase': 'Borrar',
      'ui.pencil': 'Notas',
      'ui.newGame': 'Nueva partida',
      'ui.newBoard': 'Nuevo tablero',
      'ui.again': 'Otra partida',
      'ui.retry': 'Reintentar',
      'ui.restart': 'Reiniciar',
      'ui.next': 'Siguiente',
      'ui.difficulty': 'Dificultad',
      'ui.making': 'Generando…',
      'ui.easy': 'Fácil',
      'ui.medium': 'Media',
      'ui.hard': 'Difícil',
      'ui.small': 'Pequeño',
      'ui.large': 'Grande',
      'ui.boardSize': 'Tamaño del tablero',
      'record.best': 'Mejor {time}',
      'record.done': '¡Completado! {time}',
      'record.hinted': 'Usaste {count} pistas, así que no cuenta para el récord.',
      'record.improved': 'Has bajado tu mejor marca de {time}.',
      'record.first': 'Tu primera marca.',
      'record.bestIs': 'Tu mejor marca es {time}.',
      'record.genFail': 'No se pudo generar el tablero. Pulsa Nuevo tablero para reintentar.',
      'record.hintedShort': 'con pistas, no cuenta para el récord',
      'record.bestMark': 'mejor marca',
      'ui.toStart': 'Desde el principio',
      'ui.nextBoard': 'Siguiente tablero',
      'ui.players': 'Jugadores',
      'ui.playerCount': '{n} jugadores',

      'daily.toast': 'Reto diario {done}/{total}',
      'daily.toastAll': '¡Reto diario completado!',
      'daily.sudoku.any': 'Resuelve uno en cualquier nivel',
      'daily.sudoku.medium': 'Resuelve en Medio o más difícil',
      'daily.sudoku.fast': 'Resuelve en menos de {min} minutos',
      'daily.sudoku.hard': 'Resuelve en Difícil',
      'daily.queens.any': 'Resuelve un tablero',
      'daily.queens.big': 'Resuelve un tablero de {size}×{size}',
      'daily.queens.clean': 'Resuelve sin pistas',
      'daily.queens.fast': 'Resuelve {size}×{size} o mayor sin pistas en menos de {min} min',
      'daily.tango.any': 'Resuelve un tablero',
      'daily.tango.big': 'Resuelve un tablero de {size}×{size}',
      'daily.tango.clean': 'Resuelve sin pistas',
      'daily.tango.fast': 'Resuelve {size}×{size} sin pistas en menos de {min} min',
      'daily.nonogram.any': 'Resuelve uno de {size}×{size} o mayor',
      'daily.nonogram.clean': 'Resuelve {size}×{size} o mayor sin pistas',
      'daily.nonogram.big': 'Resuelve uno de {size}×{size}',
      'daily.nonogram.hard': 'Resuelve {size}×{size} sin pistas'
    }
  });
})();
