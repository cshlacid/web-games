...
// 때문이다** — 판 넷(2048·Zip·Queens·Patches), 힐러의 전장, 시트 손잡이가
...
// 못한다. Queens가 그랬고, 그래서 그 게임은 그림을 미리 넣어 두고 감춘다.
(function () {
...
// **`none`이 아닌 자리에도 그물을 친다.** CSS가 대부분을 막지만 사파리 판에 따라
// 새는 자리가 나왔다(꼬들의 글자판, 힐러의 흰 판). 여기서는 조건을 건다 — 조건
// 없이 막으면 뒤따르는 click이 나지 않아 단추가 죽는다.
//
...
// `[tabindex]`는 넣지 않는다. 시트 겹판이 초점을 받으려고 `tabindex="-1"`을 달고
// 있어, 넣으면 도움말 안이 통째로 그물에서 빠진다. 초점을 받는 것과 누르는 것은
// 다르다.
const PRESSABLE = 'button, a, input, select, textarea, label, summary, [role="button"]';
const GAP = 700;
const NEAR = 40;
